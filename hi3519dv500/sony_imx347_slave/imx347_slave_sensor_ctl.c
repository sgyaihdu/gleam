...
static void imx347_slave_linear_4m60_init(ot_vi_pipe vi_pipe);
...
void imx347_slave_init(ot_vi_pipe vi_pipe)
{
    ...
    if (init == TD_FALSE) {
        if (OT_WDR_MODE_NONE == wdr_mode) {
            ...
            }else if (IMX347_SLAVE_SENSOR_4M_60FPS_LINEAR_MODE == img_mode){
                imx347_slave_linear_4m60_init(vi_pipe);
            }
        }
    } else {
...
static td_s32 imx347_slave_linear_4m60_part1(ot_vi_pipe vi_pipe)
{
    td_s32 ret = TD_SUCCESS;
    ret += imx347_slave_write_register(vi_pipe, 0x3000, 0x01);  /* STANDBY */
    ret += imx347_slave_write_register(vi_pipe, 0x3002, 0x01);  /* XTMSTA */
    ...
    ret += imx347_slave_write_register(vi_pipe, 0x300C, 0x5B);
    ret += imx347_slave_write_register(vi_pipe, 0x300D, 0x40);
    ret += imx347_slave_write_register(vi_pipe, 0x3030, 0xAB);
    ret += imx347_slave_write_register(vi_pipe, 0x3031, 0x09);
    ret += imx347_slave_write_register(vi_pipe, 0x3034, 0xF4);
    ret += imx347_slave_write_register(vi_pipe, 0x3035, 0x01);
    ret += imx347_slave_write_register(vi_pipe, 0x3050, 0x00);
    ret += imx347_slave_write_register(vi_pipe, 0x30BE, 0x5E);
    ret += imx347_slave_write_register(vi_pipe, 0x315A, 0x02);
    ret += imx347_slave_write_register(vi_pipe, 0x316A, 0x7E);
    ret += imx347_slave_write_register(vi_pipe, 0x319D, 0x00);
    ret += imx347_slave_write_register(vi_pipe, 0x31A1, 0x00);
    ret += imx347_slave_write_register(vi_pipe, 0x3202, 0x02);
    ret += imx347_slave_write_register(vi_pipe, 0x3288, 0x22);
    ret += imx347_slave_write_register(vi_pipe, 0x328A, 0x02);
    ret += imx347_slave_write_register(vi_pipe, 0x328C, 0xA2);
...
static void imx347_slave_linear_4m60_init(ot_vi_pipe vi_pipe)
{
    ...
    td_s32 ret = TD_SUCCESS;
    ret += imx347_slave_linear_4m60_part1(vi_pipe);
    //ret += imx347_slave_linear_4m30_part2(vi_pipe);
    ...
    printf("===IMX347 Slave 4M 60fps 12bit LINE Init OK!===\n");
    return;
}
