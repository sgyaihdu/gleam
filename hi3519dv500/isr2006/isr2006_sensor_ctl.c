...
{
    if (memset_s(buf, buf_size, 0, ISR2006_ADDR_BYTE) != EOK)
    {
...
// {
//     if (g_fd[vi_pipe] < 0) {
//         printf("isr2006_read_register fd not opened!\n");
//         return TD_FAILURE;
//     }
//     struct i2c_rdwr_args args;
//     td_u32 cur_addr;
//     td_u32 data;
//     unsigned char buf[ISR2006_ADDR_BYTE];
//     static struct i2c_rdwr_ioctl_data rdwr;
//     static struct i2c_msg msg[ISR2006_ADDR_BYTE];
//     rdwr.msgs = &msg[0];
//     rdwr.nmsgs = (__u32)ISR2006_ADDR_BYTE;
//     args.i2c_num = 5;
//     args.dev_addr = (ISR2006_I2C_ADDR);
//     args.reg_addr = addr;
//     args.reg_addr_end = addr;
//     args.reg_width = ISR2006_ADDR_BYTE;
//     args.data_width = ISR2006_DATA_BYTE;
//     args.reg_step = 1;
...
//             buf[0] = cur_addr & 0xff;
//         }
//         if (ioctl(g_fd[vi_pipe], OT_I2C_RDWR, &rdwr) != I2C_READ_STATUS_OK) {
...
td_s32 isr2006_read_and_check_register(ot_vi_pipe vi_pipe, td_u32 addr, td_u32 data)
{
    // td_u32 idx = 0;
    td_s32 retval;
    td_u8 buf[I2C_BUF_NUM] = {0};
    // printf();
    td_u32 tmp_data = 0;

    static struct i2c_rdwr_ioctl_data rdwr = {0};
    static struct i2c_msg msg[2] = {0};
    msg[0].addr = ISR2006_I2C_ADDR;
    msg[0].flags = 0;
    msg[0].len = ISR2006_ADDR_BYTE;
    msg[0].buf = buf;
    msg[1].addr = ISR2006_I2C_ADDR;
    msg[1].flags = 0;
    msg[1].flags |= I2C_M_RD;
    msg[1].len = ISR2006_DATA_BYTE;
    msg[1].buf = buf;
    ...
    retval = ioctl(g_fd[vi_pipe], OT_I2C_RDWR, &rdwr);
    if (retval != 2)
    {
        printf("CMD_I2C_READ error!\n");
        retval = -1;
    ...
    if (data != tmp_data)
    {
        // printf("i2c input %#x and out %#x is not same\n",data,tmp_data);
    }
    retval = 0;
end:
    return retval;
...
td_s32 isr2006_write_register(ot_vi_pipe vi_pipe, td_u32 addr, td_u32 data)
{
    ...
    delay_ms(1);
    isr2006_read_and_check_register(vi_pipe, addr, data);
...
void isr2006_linear_init(ot_vi_pipe vi_pipe);
...
// void isr2006_read_and_print_all_registers(ot_vi_pipe vi_pipe) {
//         td_u32 reg_addr[] = {
//         0x007f, 0x0082, 0x0083, 0x0084, 0x0086, 0x0089, 0x008b, 0x01e0,
//         0x01e2, 0x00ac, 0x00ad, 0x01e5, 0x01e6, 0x01e7, 0x01e8, 0x00a5,
//         0x00a7, 0x00a4, 0x00a9, 0x00aa, 0x00ab, 0x008f, 0x0090, 0x0091,
//         0x0092, 0x0093, 0x00e2, 0x014f, 0x0150, 0x01d7, 0x01d6, 0x0058,
//         0x0059, 0x005a, 0x005b, 0x1201, 0x1202, 0x1203, 0x1204, 0x1070,
//         0x1205, 0x1208, 0x1000, 0x1001, 0x1024, 0x1025, 0x1026, 0x1027,
//         0x1040, 0x1020, 0x1042, 0x1028, 0x1029, 0x102a, 0x102b, 0x102c,
//         0x102d, 0x102e, 0x102f, 0x1030, 0x1031, 0x1032, 0x1033, 0x01e3,
//         0x00a4, 0x1040, 0x005b, 0x005d, 0x0042, 0x009b, 0x009c, 0x009d,
//         0x009e, 0x00a5, 0x00a6, 0x0261, 0x0262, 0x002b, 0x002c, 0x0030,
//         0x01d7, 0x01d8, 0x0045, 0x0046, 0x0150, 0x0090, 0x008f, 0x0027,
//         0x0025
//     };
//     int num_registers = sizeof(reg_addr) / sizeof(reg_addr[0]);

//     for (int i = 0; i < num_registers; i++) {
//         td_u32 reg_val = 0;
//         int ret = isr2006_read_register(vi_pipe, reg_addr[i]);
//         if (ret != I2C_READ_ERROR) {
//             printf("Reg 0x%x: 0x%x\n", reg_addr[i], reg_val);
//         } else {
//             printf("Error reading register 0x%x\n", reg_addr[i]);
//         }
//     }
// }
...
void isr2006_init(ot_vi_pipe vi_pipe)
{
    ...
    if (init == TD_FALSE)
    {
        ...
        {
            isr2006_linear_init(vi_pipe); // 初始化了时钟 输出模式为mipi 输出格式为12bit等
        }
    }
    else
    {
...
static td_s32 isr2006_linear_1936_1280_init_part(ot_vi_pipe vi_pipe)
{
    ...
    ret += isr2006_write_register(vi_pipe, 0x0083, 0xc0); // pll_fbdiv = 0xc0   = 192
    ...
    // ret += isr2006_write_register(vi_pipe, 0x00dc, 0x00);  // 暗像素 起始位置

    // ret += isr2006_write_register(vi_pipe, 0x00de, 0x28); // 暗像素 列数

    // ret += isr2006_write_register(vi_pipe, 0x00d0, 0x01);

    // ret += isr2006_write_register(vi_pipe, 0x00d3, 0x00);

    // ret += isr2006_write_register(vi_pipe, 0x01de, 0x01); //暗像素使能
...
td_s32 isr2006_linear_1936_1280_25fps_FOT_init_part(ot_vi_pipe vi_pipe)
{
    td_s32 ret = TD_SUCCESS;
    printf("===ISR2006 1936*1280 25fps 12bit FOT LINE Init Part===\n");
    ret += isr2006_write_register(vi_pipe, 0x007f, 0x00); 
    ret += isr2006_write_register(vi_pipe, 0x0082, 0x04); 
    ret += isr2006_write_register(vi_pipe, 0x0083, 0xc0); 
    ret += isr2006_write_register(vi_pipe, 0x0084, 0x01); 
    ret += isr2006_write_register(vi_pipe, 0x0086, 0x01); 
    ret += isr2006_write_register(vi_pipe, 0x0089, 0x05); 
    ret += isr2006_write_register(vi_pipe, 0x008b, 0x05); 
    ret += isr2006_write_register(vi_pipe, 0x01df, 0x00); 
    ret += isr2006_write_register(vi_pipe, 0x01e0, 0x00); 
    ret += isr2006_write_register(vi_pipe, 0x01e1, 0x8f); 
    ret += isr2006_write_register(vi_pipe, 0x01e2, 0x07); 
    ret += isr2006_write_register(vi_pipe, 0x00ac, 0x00); 
    ret += isr2006_write_register(vi_pipe, 0x00ad, 0x05); 
    ret += isr2006_write_register(vi_pipe, 0x01e5, 0x00); 
    ret += isr2006_write_register(vi_pipe, 0x01e6, 0x00); 
    ret += isr2006_write_register(vi_pipe, 0x01e7, 0x01); 
    ret += isr2006_write_register(vi_pipe, 0x01e8, 0x01); 
    ret += isr2006_write_register(vi_pipe, 0x00a5, 0x00); 
    ret += isr2006_write_register(vi_pipe, 0x00a7, 0x00); 
    ret += isr2006_write_register(vi_pipe, 0x00a4, 0x00); 
    ret += isr2006_write_register(vi_pipe, 0x00a9, 0x01); 
    ret += isr2006_write_register(vi_pipe, 0x00aa, 0x00); 
    ret += isr2006_write_register(vi_pipe, 0x00ab, 0x2d); 
    ret += isr2006_write_register(vi_pipe, 0x008f, 0x4e); 
    ret += isr2006_write_register(vi_pipe, 0x0090, 0x09); //30ms
    ret += isr2006_write_register(vi_pipe, 0x0091, 0x00); 
    ret += isr2006_write_register(vi_pipe, 0x00e2, 0x00); 
    ret += isr2006_write_register(vi_pipe, 0x014f, 0x00); 
    ret += isr2006_write_register(vi_pipe, 0x0150, 0x01); 
    ret += isr2006_write_register(vi_pipe, 0x01d7, 0x00); 
    ret += isr2006_write_register(vi_pipe, 0x01d6, 0x00); 
    ret += isr2006_write_register(vi_pipe, 0x0058, 0x01); 
    ret += isr2006_write_register(vi_pipe, 0x0059, 0x0f); 
    ret += isr2006_write_register(vi_pipe, 0x005a, 0x01); 
    ...
    ret += isr2006_write_register(vi_pipe, 0x1205, 0x00); 
    ret += isr2006_write_register(vi_pipe, 0x1208, 0x01); 
    ret += isr2006_write_register(vi_pipe, 0x1000, 0x08); 
    ret += isr2006_write_register(vi_pipe, 0x1001, 0x00); 
    ret += isr2006_write_register(vi_pipe, 0x1070, 0x12); 
    ret += isr2006_write_register(vi_pipe, 0x1070, 0x02); 
    ret += isr2006_write_register(vi_pipe, 0x1024, 0x90); 
    ret += isr2006_write_register(vi_pipe, 0x1025, 0x07); 
    ret += isr2006_write_register(vi_pipe, 0x1026, 0x00); 
    ret += isr2006_write_register(vi_pipe, 0x1027, 0x05); 
    ret += isr2006_write_register(vi_pipe, 0x1040, 0x8d); 
    ret += isr2006_write_register(vi_pipe, 0x1020, 0x2a); 
    ret += isr2006_write_register(vi_pipe, 0x1042, 0x0f); 
    ret += isr2006_write_register(vi_pipe, 0x1028, 0x90); 
    ret += isr2006_write_register(vi_pipe, 0x1029, 0x07); 
    ret += isr2006_write_register(vi_pipe, 0x102a, 0x00); 
    ret += isr2006_write_register(vi_pipe, 0x102b, 0x05); 
    ret += isr2006_write_register(vi_pipe, 0x102c, 0x90); 
    ret += isr2006_write_register(vi_pipe, 0x102d, 0x07); 
    ret += isr2006_write_register(vi_pipe, 0x102e, 0x00); 
    ret += isr2006_write_register(vi_pipe, 0x102f, 0x05); 
    ret += isr2006_write_register(vi_pipe, 0x1030, 0x90); 
    ret += isr2006_write_register(vi_pipe, 0x1031, 0x07); 
    ret += isr2006_write_register(vi_pipe, 0x1032, 0x00); 
    ret += isr2006_write_register(vi_pipe, 0x1033, 0x05); 
    ret += isr2006_write_register(vi_pipe, 0x01e3, 0x1e); 
    ret += isr2006_write_register(vi_pipe, 0x00a4, 0x01); 
    ret += isr2006_write_register(vi_pipe, 0x1040, 0x8c); 
    ret += isr2006_write_register(vi_pipe, 0x005b, 0x03); 
    ret += isr2006_write_register(vi_pipe, 0x005d, 0x0f); 
    ret += isr2006_write_register(vi_pipe, 0x008f, 0x00); 
    ret += isr2006_write_register(vi_pipe, 0x0090, 0x00); 
    ret += isr2006_write_register(vi_pipe, 0x0091, 0x00); 
    ret += isr2006_write_register(vi_pipe, 0x0042, 0xaa); 
    ret += isr2006_write_register(vi_pipe, 0x0261, 0x0f); //改善HCG模式下左右噪声不一致
    ret += isr2006_write_register(vi_pipe, 0x0262, 0x81); //改善HCG模式下左右噪声不一致
    ret += isr2006_write_register(vi_pipe, 0x0065, 0x05); //消除LCG模式下的坏点
    ret += isr2006_write_register(vi_pipe, 0x009b, 0x01); 
    ret += isr2006_write_register(vi_pipe, 0x009c, 0x01); 
    ret += isr2006_write_register(vi_pipe, 0x009d, 0x01); 
    ret += isr2006_write_register(vi_pipe, 0x009e, 0x01); //LCG模式

    // ret += isr2006_write_register(vi_pipe, 0x002b, 0x70);
    // ret += isr2006_write_register(vi_pipe, 0x002c, 0x01);
    // ret += isr2006_write_register(vi_pipe, 0x002b, 0x2c);//32xadc

    // ret += isr2006_write_register(vi_pipe, 0x01d7, 0x01);
    // ret += isr2006_write_register(vi_pipe, 0x01d8, 0x1f); //32xdigital


    ret += isr2006_write_register(vi_pipe, 0x0247, 0x1e); 
    ret += isr2006_write_register(vi_pipe, 0x023f, 0x22); 
    ret += isr2006_write_register(vi_pipe, 0x021f, 0x14); 
    ret += isr2006_write_register(vi_pipe, 0x01f7, 0x16); 
    ret += isr2006_write_register(vi_pipe, 0x01ed, 0x01); 
    ret += isr2006_write_register(vi_pipe, 0x024f, 0x24); 
    ret += isr2006_write_register(vi_pipe, 0x0237, 0x26); 
    ret += isr2006_write_register(vi_pipe, 0x022f, 0x2a); 
    ret += isr2006_write_register(vi_pipe, 0x0255, 0x2d); 
    ret += isr2006_write_register(vi_pipe, 0x0257, 0x3d); 
    ret += isr2006_write_register(vi_pipe, 0x026d, 0x3d); 
    ret += isr2006_write_register(vi_pipe, 0x026f, 0x3e); 
    ret += isr2006_write_register(vi_pipe, 0x0265, 0x3d); 
    ret += isr2006_write_register(vi_pipe, 0x025d, 0x3e); 
    ret += isr2006_write_register(vi_pipe, 0x025f, 0x5c); 
    ret += isr2006_write_register(vi_pipe, 0x0263, 0xd4); 
    ret += isr2006_write_register(vi_pipe, 0x0264, 0x01); 
    ret += isr2006_write_register(vi_pipe, 0x0227, 0x5c); 
    ret += isr2006_write_register(vi_pipe, 0x01ef, 0x5c); 
    ret += isr2006_write_register(vi_pipe, 0x01f1, 0x78); 
    ret += isr2006_write_register(vi_pipe, 0x01f3, 0xd5); 
    ret += isr2006_write_register(vi_pipe, 0x01f4, 0x01); 
    ret += isr2006_write_register(vi_pipe, 0x01fd, 0x5d); 
    ret += isr2006_write_register(vi_pipe, 0x01ff, 0x72); 
    ret += isr2006_write_register(vi_pipe, 0x0275, 0xd4); 
    ret += isr2006_write_register(vi_pipe, 0x0276, 0x81); 
    ret += isr2006_write_register(vi_pipe, 0x0277, 0xd5); 
    ret += isr2006_write_register(vi_pipe, 0x0278, 0x01); 
    ret += isr2006_write_register(vi_pipe, 0x00a5, 0x01); 
    ret += isr2006_write_register(vi_pipe, 0x00a6, 0x01); 
...
{
    td_s32 ret = TD_SUCCESS;
    printf("===ISR2006 1936*1280 25fps 12bit LINE Init Part===\n");
    ...
    ret += isr2006_write_register(vi_pipe, 0x008b, 0x35); // 35d
...
td_s32 isr2006_linear_1936_1280_30fps_init_part(ot_vi_pipe vi_pipe)
{
    td_s32 ret = TD_SUCCESS;
    printf("===ISR2006 1936*1280 30fps 12bit LINE Init Part===\n");
    ret += isr2006_write_register(vi_pipe, 0x007f, 0x00); // 0d
    ret += isr2006_write_register(vi_pipe, 0x0082, 0x04); // 4d
    ret += isr2006_write_register(vi_pipe, 0x0083, 0xc0); // 192d
    ret += isr2006_write_register(vi_pipe, 0x0084, 0x05); // 5d
    ret += isr2006_write_register(vi_pipe, 0x0086, 0x03); // 3d
    ret += isr2006_write_register(vi_pipe, 0x0089, 0x05); // 5d
    ret += isr2006_write_register(vi_pipe, 0x008b, 0x35); // 35d
    ...
    ret += isr2006_write_register(vi_pipe, 0x005d, 0x0f); // 15d
    ret += isr2006_write_register(vi_pipe, 0x0042, 0xaa); // 170d
    ret += isr2006_write_register(vi_pipe, 0x009b, 0x01); // 0d // 9b 9c 9d 9e 写0为HCG 写1为LCG
    ret += isr2006_write_register(vi_pipe, 0x009c, 0x01); // 0d
    ret += isr2006_write_register(vi_pipe, 0x009d, 0x01); // 0d
    ret += isr2006_write_register(vi_pipe, 0x009e, 0x01); // 0d
    ret += isr2006_write_register(vi_pipe, 0x00a5, 0x01); // 1d
    ret += isr2006_write_register(vi_pipe, 0x00a6, 0x01); // 1d
    ret += isr2006_write_register(vi_pipe, 0x00a6, 0x00); // 0d
    ret += isr2006_write_register(vi_pipe, 0x0261, 0x0f); // 15d //调整列条纹
...
td_s32 isr2006_linear_1936_1280_60fps_init_part(ot_vi_pipe vi_pipe)
{
    td_s32 ret = TD_SUCCESS;
    printf("===ISR2006 1936*1280 60fps 12bit LINE Init Part===\n");
    ret += isr2006_write_register(vi_pipe, 0x007f, 0x00); // 0d
    ret += isr2006_write_register(vi_pipe, 0x0082, 0x04); // 4d
    ret += isr2006_write_register(vi_pipe, 0x0083, 0xc0); // 192d
    ret += isr2006_write_register(vi_pipe, 0x0084, 0x05); // 5d
    ret += isr2006_write_register(vi_pipe, 0x0086, 0x08); // 8d
    ret += isr2006_write_register(vi_pipe, 0x0089, 0x05); // 5d
    ret += isr2006_write_register(vi_pipe, 0x008b, 0x35); // 35d
...
td_s32 isr2006_linear_1920_1080_120fps_init_part(ot_vi_pipe vi_pipe)
{
    td_s32 ret = TD_SUCCESS;
    printf("===ISR2006 1920*1080 120fps 10bit LINE Init Part===\n");
    ret += isr2006_write_register(vi_pipe, 0x007f, 0x00);
    ret += isr2006_write_register(vi_pipe, 0x0082, 0x02);
    ret += isr2006_write_register(vi_pipe, 0x0083, 0xd4);
    ret += isr2006_write_register(vi_pipe, 0x0084, 0x04);
    ret += isr2006_write_register(vi_pipe, 0x0086, 0x01);
    ret += isr2006_write_register(vi_pipe, 0x0089, 0x01);
    ret += isr2006_write_register(vi_pipe, 0x008b, 0x35);
    ret += isr2006_write_register(vi_pipe, 0x01eb, 0xf4);
    ret += isr2006_write_register(vi_pipe, 0x01ec, 0x01);
    ret += isr2006_write_register(vi_pipe, 0x01df, 0x00);
    ret += isr2006_write_register(vi_pipe, 0x01e0, 0x00);
    ret += isr2006_write_register(vi_pipe, 0x01e1, 0x8f);
    ret += isr2006_write_register(vi_pipe, 0x01e2, 0x07);
    ...
    ret += isr2006_write_register(vi_pipe, 0x01e5, 0x00);
    ret += isr2006_write_register(vi_pipe, 0x01e6, 0x00);
    ret += isr2006_write_register(vi_pipe, 0x01e7, 0x00);
    ret += isr2006_write_register(vi_pipe, 0x01e8, 0x00);
    ret += isr2006_write_register(vi_pipe, 0x00a5, 0x00);
    ret += isr2006_write_register(vi_pipe, 0x00a7, 0x00);
    ret += isr2006_write_register(vi_pipe, 0x00a4, 0x00);
    ret += isr2006_write_register(vi_pipe, 0x00a9, 0x01);
    ret += isr2006_write_register(vi_pipe, 0x00aa, 0x00);
    ret += isr2006_write_register(vi_pipe, 0x00ab, 0x2d);
    ret += isr2006_write_register(vi_pipe, 0x008f, 0x08);
    ret += isr2006_write_register(vi_pipe, 0x0090, 0x00);
    ret += isr2006_write_register(vi_pipe, 0x0091, 0x00);
    ret += isr2006_write_register(vi_pipe, 0x00e2, 0x00);
    ret += isr2006_write_register(vi_pipe, 0x014f, 0x00);
    ...
    ret += isr2006_write_register(vi_pipe, 0x005b, 0x03);
    ret += isr2006_write_register(vi_pipe, 0x005d, 0x0f);
    ret += isr2006_write_register(vi_pipe, 0x008f, 0x1a);
    ret += isr2006_write_register(vi_pipe, 0x0090, 0x00);
    ret += isr2006_write_register(vi_pipe, 0x0091, 0x00);
    ret += isr2006_write_register(vi_pipe, 0x0042, 0xaa);
    ret += isr2006_write_register(vi_pipe, 0x0065, 0x05);
    ...
    ret += isr2006_write_register(vi_pipe, 0x0141, 0x03);
    ret += isr2006_write_register(vi_pipe, 0x00a5, 0x01);
    ret += isr2006_write_register(vi_pipe, 0x00a6, 0x01);
    ret += isr2006_write_register(vi_pipe, 0x00a6, 0x00);
...
td_s32 isr2006_linear_968_640_25fps_init_part(ot_vi_pipe vi_pipe)
{
    td_s32 ret = TD_SUCCESS;
    ret += isr2006_write_register(vi_pipe, 0x007f, 0x00); // 0d
    ret += isr2006_write_register(vi_pipe, 0x0082, 0x04); // 4d
    ...
    ret += isr2006_write_register(vi_pipe, 0x01df, 0x00);
    ret += isr2006_write_register(vi_pipe, 0x01e0, 0x00);
    ret += isr2006_write_register(vi_pipe, 0x01e1, 0xC8);
    ret += isr2006_write_register(vi_pipe, 0x01e2, 0x03);

    /*0x00ac 0x00ad 开窗总行数   640 */

    ret += isr2006_write_register(vi_pipe, 0x00ac, 0x80); // 0d
    ret += isr2006_write_register(vi_pipe, 0x00ad, 0x02); // 5d

    ret += isr2006_write_register(vi_pipe, 0x00ae, 0x00);
    ret += isr2006_write_register(vi_pipe, 0x00af, 0x00);
    ret += isr2006_write_register(vi_pipe, 0x00b0, 0x80);
    ret += isr2006_write_register(vi_pipe, 0x00b1, 0x02);
...
td_s32 isr2006_linear_1920_1080_30fps_14bit_init_part(ot_vi_pipe vi_pipe)
{
    td_s32 ret = 0;
    ret += isr2006_write_register(vi_pipe, 0x007f, 0x00);
    ret += isr2006_write_register(vi_pipe, 0x0082, 0x01);
    ret += isr2006_write_register(vi_pipe, 0x0083, 0x54);
    ret += isr2006_write_register(vi_pipe, 0x0084, 0x01);
    ret += isr2006_write_register(vi_pipe, 0x0086, 0x01);
    ret += isr2006_write_register(vi_pipe, 0x0089, 0x0B);
    ret += isr2006_write_register(vi_pipe, 0x008b, 0x02);
    ret += isr2006_write_register(vi_pipe, 0x01df, 0x00);
    ret += isr2006_write_register(vi_pipe, 0x01e0, 0x00);
    ret += isr2006_write_register(vi_pipe, 0x01e1, 0x80);
    ret += isr2006_write_register(vi_pipe, 0x01e2, 0x07);
    ret += isr2006_write_register(vi_pipe, 0x00ac, 0x38);
    ret += isr2006_write_register(vi_pipe, 0x00ad, 0x04);
    ret += isr2006_write_register(vi_pipe, 0x00ae, 0x00);
    ret += isr2006_write_register(vi_pipe, 0x00af, 0x00);
    ret += isr2006_write_register(vi_pipe, 0x00b0, 0x38);
    ret += isr2006_write_register(vi_pipe, 0x00b1, 0x04);
    ret += isr2006_write_register(vi_pipe, 0x01e5, 0x00);
    ret += isr2006_write_register(vi_pipe, 0x01e6, 0x00);
    ret += isr2006_write_register(vi_pipe, 0x01e7, 0x02);
    ret += isr2006_write_register(vi_pipe, 0x01e8, 0x02);
    ret += isr2006_write_register(vi_pipe, 0x00a5, 0x00);
    ret += isr2006_write_register(vi_pipe, 0x00a7, 0x00);
    ret += isr2006_write_register(vi_pipe, 0x00a4, 0x00);
    ret += isr2006_write_register(vi_pipe, 0x00a9, 0x04);
    ret += isr2006_write_register(vi_pipe, 0x00aa, 0x00);
    ret += isr2006_write_register(vi_pipe, 0x008f, 0x08);
    ret += isr2006_write_register(vi_pipe, 0x0090, 0x00);
    ret += isr2006_write_register(vi_pipe, 0x0091, 0x00);
    ret += isr2006_write_register(vi_pipe, 0x00e2, 0x00);
    ret += isr2006_write_register(vi_pipe, 0x014f, 0x00);
    ret += isr2006_write_register(vi_pipe, 0x0150, 0x00);
    ret += isr2006_write_register(vi_pipe, 0x01d7, 0x00);
    ret += isr2006_write_register(vi_pipe, 0x01d6, 0x00);
    ret += isr2006_write_register(vi_pipe, 0x0058, 0x01);
    ret += isr2006_write_register(vi_pipe, 0x0059, 0x0f);
    ...
    ret += isr2006_write_register(vi_pipe, 0x1070, 0x12);
    ret += isr2006_write_register(vi_pipe, 0x1070, 0x02);
    ret += isr2006_write_register(vi_pipe, 0x1024, 0x80);
    ret += isr2006_write_register(vi_pipe, 0x1025, 0x07);
    ret += isr2006_write_register(vi_pipe, 0x1026, 0x38);
    ret += isr2006_write_register(vi_pipe, 0x1027, 0x04);
    ret += isr2006_write_register(vi_pipe, 0x1040, 0x8d);
    ret += isr2006_write_register(vi_pipe, 0x1020, 0x2a);
    ret += isr2006_write_register(vi_pipe, 0x1042, 0x03);
    ret += isr2006_write_register(vi_pipe, 0x1028, 0x90);
    ret += isr2006_write_register(vi_pipe, 0x1029, 0x07);
    ret += isr2006_write_register(vi_pipe, 0x102a, 0x00);
    ret += isr2006_write_register(vi_pipe, 0x102b, 0x05);
    ...
    ret += isr2006_write_register(vi_pipe, 0x1040, 0x8d);
    ret += isr2006_write_register(vi_pipe, 0x005b, 0x03);
    ret += isr2006_write_register(vi_pipe, 0x005d, 0x07);
    ret += isr2006_write_register(vi_pipe, 0x008f, 0xc8);
    ret += isr2006_write_register(vi_pipe, 0x0090, 0x00);
    ret += isr2006_write_register(vi_pipe, 0x0091, 0x00);
    ret += isr2006_write_register(vi_pipe, 0x00a5, 0x01);
    ret += isr2006_write_register(vi_pipe, 0x00a6, 0x01);
...
void isr2006_linear_init(ot_vi_pipe vi_pipe)
{
    ...
    switch (img_mode)
    {
    case ISR2006_SENSOR_1936_1280_30FPS_12BIT_LINEAR_MODE:
        ret += isr2006_linear_1936_1280_30fps_init_part(vi_pipe);
        if (ret != TD_SUCCESS)
        {
    ...
    case ISR2006_SENSOR_1936_1280_25FPS_12BIT_LINEAR_MODE:
        ...
        //ret += isr2006_linear_1936_1280_25fps_FOT_init_part(vi_pipe);
        if (ret != TD_SUCCESS)
        {
            isp_err_trace("isr2006 write register failed!\n");
            return;
        ...
        break;
    case ISR2006_SENSOR_1920_1080_30FPS_14BIT_LINEAR_MODE:
        ret += isr2006_linear_1920_1080_30fps_14bit_init_part(vi_pipe);
        if (ret != TD_SUCCESS)
        {
            isp_err_trace("isr2006 write register failed!\n");
            return;
        }
        printf("===ISR2006 1936*1280 30fps 14bit LINE Init OK!===\n");
        break;
    case  ISR2006_SENSOR_1936_1280_60FPS_12BIT_LINEAR_MODE:
        ret += isr2006_linear_1936_1280_60fps_init_part(vi_pipe);
        if (ret != TD_SUCCESS)
        {
            isp_err_trace("isr2006 write register failed!\n");
            return;
        }
        printf("===ISR2006 1936*1280 60fps 12bit LINE Init OK!===\n");
        break;
    case ISR2006_SENSOR_1920_1080_120FPS_10BIT_LINEAR_MODE:
        ret += isr2006_linear_1920_1080_120fps_init_part(vi_pipe);
        if (ret != TD_SUCCESS)
        {
            isp_err_trace("isr2006 write register failed!\n");
            return;
        }
        printf("===ISR2006 1920*1080 120fps 10bit LINE Init OK!===\n");
        break;
        break;
    }
    ...
    // ret += isr_linear_mipi_time(vi_pipe);
    ...
    // #region 打印寄存器
    // int result = system("i2c_read 0x5 0xca 0x0000 0x1581 2 1 1 | grep -v \" 0x0\"");
