...
static hi_char *g_model_file = "./aibnr/model/aibnr_model_denoise_priority.bin";
...
static hi_void sample_aibnr_print_model_file_list(hi_void)
{
...
static hi_void sample_aibnr_get_model_file_by_char(hi_char ch, hi_vi_pipe vi_pipe, sample_vi_cfg *vi_cfg)
{
    switch (ch)
    {
    case '0':
...
static hi_void sample_aibnr_get_model_file(hi_vi_pipe vi_pipe, sample_vi_cfg *vi_cfg)
{
    ...
    //     if (gets_s(input, max_len) != HI_NULL && strlen(input) == 1 && input[0] >= '0' && input[0] <= '3') {
    //         break;
    //     } else {
...
static hi_s32 sample_aibnr_start(hi_vi_pipe vi_pipe, hi_size in_size, hi_aibnr_model *model_info, hi_s32 *model_id,
...
static hi_s32 sample_aibnr_stop(hi_vi_pipe vi_pipe, hi_aibnr_model *model, hi_s32 *model_id, hi_s32 model_size)
{
    hi_s32 ret1;
    hi_s32 ret2;
...
static hi_void sample_aibnr_stop_vi(hi_vi_pipe vi_pipe, const sample_vi_cfg *vi_cfg)
{
...
static hi_s32 sample_aibnr_start_vi(hi_vi_pipe vi_pipe, sample_vi_cfg *vi_cfg, hi_size *in_size,
                                    hi_u32 vb_cnt)
{
    hi_s32 ret;
    sample_aibnr_get_model_file(vi_pipe, vi_cfg); //选择模型文件
...
static hi_s32 sample_aibnr_set_blc(hi_vi_pipe vi_pipe, sample_sns_type sns_type)
{
    hi_s32 i, j, ret;
...
static hi_void sample_aibnr_update_cfg(hi_vi_pipe vi_pipe, sample_vi_cfg *vi_cfg)
{
...
hi_s32 sample_aibnr_line_switch_to_wdr(hi_void)
{
    ...
    sample_sns_type wdr_sns_type = sample_aiisp_get_wdr_sns_type(SENSOR1_TYPE);
...
hi_s32 sample_aibnr(sample_aibnr_param *aibnr_param)
{
    ...
    sample_sns_type sns_type = aibnr_param->is_wdr_mode ? sample_aiisp_get_wdr_sns_type(SENSOR1_TYPE) : SENSOR1_TYPE;
...
#if FPN_ENABLE
    g_correction_cfg.aibnr_mode = HI_TRUE;
    ret = sample_comm_vi_enable_fpn_correction(vi_pipe, &g_correction_cfg);
    if (ret != HI_SUCCESS)
    {
        printf("enable fpn correction failed\n");
    }
#endif

#if VI_CROP_IS_ENANLE
    ot_size crop_size = {ISR2006_CROP_WIDTH, ISR2006_CROP_HEIGHT};
    if (sample_aibnr_start(vi_pipe, crop_size, &model_info, model_id, model_size, aibnr_param) != TD_SUCCESS)
    {
        goto stop_venc_and_vo;
    }
#else
