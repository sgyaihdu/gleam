...
// static hi_s32 vo_i2c_mipi_tx_send_one_cmd(hi_s32 i2c_fd,mipi_tx_i2c_cmd_info *i2c_cmd_info,hi_u8 addr_len)   
// {
//     hi_s32 ret = 0;
//     td_u8 buf[I2C_ADDR_LEN+I2C_DATA_LEN] = {0};
//     td_s8 len = addr_len+I2C_DATA_LEN;
//     int i = 0;

//     buf[0] = i2c_cmd_info->reg_addr_high;
//     buf[1] = i2c_cmd_info->reg_addr_low;
//     buf[2] = i2c_cmd_info->reg_value;

//     // if((i2c_cmd_info->delay_en == HI_TRUE) && buf[2] == 0x00)
//     // {
//     //     len = I2C_ADDR_LEN;
//     //     buf[2] = 0x00;
//     // }
...
static hi_s32 vo_i2c_mipi_tx_init_screen(const sample_mipi_tx_config *tx_config)
{
    ...
    hi_s32 ret;
    char dev_a_file[16] = {0};
    char dev_b_file[16] = {0};
    char *dev_file[2] = {dev_a_file,dev_b_file};
    int loop = 0;
    hi_u8 i2c_bus_num[2] = {(tx_config->i2c_bus)[0],(tx_config->i2c_bus)[1]};
    hi_s32 i2c_fd[2] = {0};
    int i = 0;
    for(i = 0;i<tx_config->oled_num;i++)
    {
        printf("vo_i2c_bus = %x\n vo_i2c_addr = %x\n",(tx_config->i2c_bus)[i],(tx_config->i2c_addr)[i]);
        (hi_void) snprintf_s(dev_file[i], 16, 16 - 1, "/dev/i2c-%u", i2c_bus_num[i]);
        i2c_fd[i] = open(dev_file[i], O_RDWR,S_IRUSR | S_IWUSR);
        if (i2c_fd[i] < 0) {
            printf("Open /dev/ot_i2c_drv-%s error!\n", dev_file[i]);
            return HI_FAILURE;
        }
        ret = ioctl(i2c_fd[i],OT_I2C_SLAVE_FORCE,(tx_config->i2c_addr)[i]);
        if(ret < 0){
            printf("OT_I2C_SLAVE_FORCE error!\n");
            close(i2c_fd[i]);
            return HI_FAILURE;
        }
        for(loop = 0;(hi_u32)loop < (tx_config->i2c_cmd_count)[i];loop++)
        {
            ret = vo_i2c_mipi_tx_send_one_cmd(i2c_fd[i],&(((tx_config->i2c_cmd_info)[i])[loop]));
            if(ret != HI_SUCCESS)
            {
                printf("i = %d,loop(%u): MIPI_TX I2C SET CMD failed\n", i,loop);
                // close(i2c_fd[i]);
                // return HI_FAILURE;
            }
        }
        printf("i = %d: MIPI_TX I2C SET CMD success\n",i);
        close(i2c_fd[i]);
    }
    return HI_SUCCESS;
}
...
hi_s32 sample_comm_vo_bt1120_i2c_init(const sample_mipi_tx_config *tx_config)
{
    ...
    if (ret != HI_SUCCESS) {
        printf("%s,%d, vo_bt1120_i2c_init fail at ret(%d)\n", __FUNCTION__, __LINE__, ret);
        return ret;
    }
    return ret;
...
// i2c_user:
    ...
    if(tx_config->i2c_cmd_en == HI_TRUE) {
        ret = vo_i2c_mipi_tx_init_screen(tx_config);
        if (ret != HI_SUCCESS) {
            printf("%s,%d, vo_i2c_mipi_tx_init_screen fail at ret(%d)\n", __FUNCTION__, __LINE__, ret);
            return ret;
        }
    }
    else{
...
if(tx_config->i2c_lt9211_enable == HI_TRUE){
#ifdef LICHO_LT9211_CODE
        ret = sample_mipi_tx_init_lt9211(tx_config);
        if (ret != HI_SUCCESS) {
            printf("%s,%d, sample_mipi_tx_init_lt9211 fail at ret(%d)\n", __FUNCTION__, __LINE__, ret); 
            return ret;
        }
#endif
}
