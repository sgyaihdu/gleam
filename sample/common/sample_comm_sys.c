...
hi_s32 sample_comm_vo_bind_venc(hi_vo_layer vo_layer, hi_vo_chn vo_chn, hi_venc_chn venc_chn)
{
    ...
    src_chn.dev_id = vo_layer;
    src_chn.chn_id = vo_chn;

    dest_chn.mod_id = HI_ID_VENC;
    dest_chn.dev_id = 0;
    dest_chn.chn_id = venc_chn;

    check_return(hi_mpi_sys_bind(&src_chn, &dest_chn), "hi_mpi_sys_bind(VO-VENC)");
...
hi_s32 sample_comm_vo_un_bind_venc(hi_vo_layer vo_layer, hi_vo_chn vo_chn, hi_venc_chn venc_chn)
{
    ...
    check_return(hi_mpi_sys_unbind(&src_chn, &dest_chn), "hi_mpi_sys_unbind(VO-VENC)");
