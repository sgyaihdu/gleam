...
void Date_Analyzing(char *recv_buf)
{
    ...
    int j = 2;
    data_references.date[0] = 2;
    data_references.date[1] = 0;
    for(int i = 0; i < size; i++) {
        if(j == 4 || j == 7)
        {
            data_references.date[j] = 12;
            j++;
        }
        data_references.date[j] = data[i] - '0';
        j++;
    }
    printf("日期 %s\n 分解后: ", data);
    for(int i = 0; i <10 ; i++) {
...
{   char time[20] = {0};
    ...
    for(int i = 0; i < size; i++) {
        if(j == 2 || j == 5)
        {
            data_references.time[j] = 11;
            j++;
        }
        data_references.time[j] = time[i] - '0';
        j++;
    }
    printf("时间%s\n 分解后: ",time);
...
hi_s32 sample_comm_region_draw_cross(hi_u32 *bitmap_data, hi_u32 stride, hi_s32 x, hi_s32 y)
{
    hi_s32 ret;
    hi_u32 font_color = RGN_ARGB8888_WHITE;
    int row, col, i;

    int gap = 10; // 十字线和中心点之间的间隙
    int line_length = 20; // 十字线长度
    int width = 1920;
    int height = 1080;
    ...
    for (int j = -2; j <= 2; ++j) {
        if ((x + i) >= 0 && (x + i) < width && (y + j) >= 0 && (y + j) < height) {
            if(i*i + j*j <= 9){
                hi_u32 data_idx = (y + j) * (stride / 4) + (x + i);
    ...
    // 绘制十字线
    // 下
    for (int j = gap; j <= gap + line_length; ++j) {
    if ((y + j) >= 0 && (y + j) < height) {
        hi_u32 data_idx = (y + j) * (stride / 4) + x;
        bitmap_data[data_idx] = font_color;
        bitmap_data[data_idx+1] = font_color;
        bitmap_data[data_idx-1] = font_color;
    }
    }
    // 上
    ...
    // 右
    for (int i = gap; i <= gap + line_length; ++i) {
    if ((x + i) >= 0 && (x + i) < width) {
        hi_u32 data_idx = y * (stride / 4) + (x + i);
        bitmap_data[data_idx] = font_color;
        bitmap_data[data_idx + stride/4] = font_color;
        bitmap_data[data_idx - stride/4] = font_color;
    }
    }
    // 左
...
hi_void* sample_comm_region_draw_number(void* arg) 
{
 ...
 //   sample_print("Region handle: %p\n", handle);  // 打印句柄的值
    ...
    while (!g_exit_flag) {
        ...
        ret = sample_comm_region_draw_cross(bitmap_data, stride, 1908/2, 1152/2);
        printf("draw cross\n");
        // 绘制日期
